...
library(ggplot2)
devtools::load_all()
load("../simulation/simu_qte_v5.Rdata")
```
...
We calculate the true value for different `xi` from 0.35 to 0.85. 

```{r}
est0 %>% mutate_all( round, digits = 2) %>% show_db()
```
...
All proposed methods performs well under true value except we only use propensity score. Here we display the results when `xi = 0.5`. 
```{r}
est1 %>% ungroup() %>% subset(xi == 0.5 & km_rho == "rho_km" & scenario == 1) %>%
  mutate_if(is.numeric, round, 3)  %>% show_db()
```

### Scenario 1: under true model 
...
### Scenario 2: incorrect outcome regression 

We consider a situation when the outcome regression is not correct. 

The outcome data $Y$ is simulated from `y <- rnorm(n, 0.4 + 0.8 * a + 0.3 * x + (x^2 - 0.25), sd = 1)`, where $X_2 \sim \mathcal{N}(0, 1)$. 
...
### Scenario 3: incorrect propensity score

We consider a situation when the propensity score model is not correct. 

The treatment group is simulated from `prob <- inv_logit(-0.25 + 0.5 * x + (x^2 - 0.25))`, where $X_2 \sim \mathcal{N}(0, 1)$. Everything else are the same. 
