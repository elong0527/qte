...
simu_data <- function(n,
  ...
  if(censoring){
    ...
    if(misspecify_censor){
      censor_time <- rexp(n, rate = exp(censoring_intercept + censoring_slope * x + (x^2 - 0.25)))
    }else{
      censor_time <- rexp(n, rate = exp(censoring_intercept + censoring_slope * x))
    }
...
db <- simu_data(10000, censoring = FALSE, censoring_slope = 0.8) %>% db_prepare()
meta_true[[1]] <- map_df(xi, summary_simu_data, db = db)
res_true[[1]] <- run_simu(db, par)

db <- simu_data(10000, censoring = FALSE, censoring_slope = 1) %>% db_prepare()
meta_true[[2]] <- map_df(xi, summary_simu_data, db = db)
res_true[[2]] <- run_simu(db, par)

db <- simu_data(10000, censoring = FALSE, censoring_slope = 2) %>% db_prepare()
meta_true[[3]] <- map_df(xi, summary_simu_data, db = db)
res_true[[3]] <- run_simu(db, par)

db <- simu_data(10000, censoring = FALSE, censoring_slope = 3) %>% db_prepare()
...
res <- list()
meta <- list()
db <- simu_data(n, censoring = censoring, censoring_slope = 0.8) %>% db_prepare()
meta[[1]] <- map_df(xi, summary_simu_data, db = db)
res[[1]] <- run_simu(db, par)

db <- simu_data(n, censoring = censoring, censoring_slope = 1) %>% db_prepare()
meta[[2]] <- map_df(xi, summary_simu_data, db = db)
res[[2]] <- run_simu(db, par)

db <- simu_data(n, censoring = censoring, censoring_slope = 2) %>% db_prepare()
meta[[3]] <- map_df(xi, summary_simu_data, db = db)
res[[3]] <- run_simu(db, par)

db <- simu_data(n, censoring = censoring, censoring_slope = 3) %>% db_prepare()
...
# cd /SFS/scratch/zhanyilo/qte7
# module add R/4.0.2
# rm *
# qsub -t 1:1000 ~/runr.sh ~/qte/simulation/simu_qte_v7.R
...
#
# db <- simu_data(400)
# save(result_meta, est0, est1, db, g1, g2, g3, g4, s1, s2, s3, s4, file = "simulation/simu_qte_v7.Rdata")
